...
public class StreamTest1 {
    ...
    public static void main(String[] args) {
        ...
        students.stream().collect(Collectors.maxBy(Comparator.comparingInt(Student::getScore)))
                .ifPresent(System.out::println);
        ...
        Map<String, List<Integer>> map6 = students.stream()
                .collect(Collectors.groupingBy(Student::getName, Collectors.collectingAndThen(Collectors.toList(),
                        list -> list.stream().map(Student::getScore).collect(Collectors.toList()))));
